...
function obterIntervaloSemana(offset = 0) {
  ...
  domingo.setDate(domingo.getDate() - domingo.getDay() + offset * 7);
...
function renderizarSticker(stickerId, dataInicio, dataFim, eventos) {
  const dataLabel = document.getElementById(`data-${stickerId}`);
  const lista = document.getElementById(`lista-${stickerId}`);
  if (!dataLabel || !lista) return;

  dataLabel.textContent = `(${formatarData(dataInicio)} - ${formatarData(dataFim)})`;
  lista.innerHTML = "";
...
async function carregarRelatorio() {
  ...
  const eventosPorSemana = { passada: [], atual: [], seguinte: [] };
  ...
  for (const id of ids) {
    ...
    if (!dataInicial || !Array.isArray(eventos)) continue;
    ...
    for (const ev of eventos) {
      ...
      const semana =
        fim >= semanaPassada.inicio && fim <= semanaPassada.fim
          ? "passada"
      ...
      if (semana) {
        eventosPorSemana[semana].push({
          titulo,
          nome: ev.evento || "Evento sem nome",
          data: fim,
        });
      }
