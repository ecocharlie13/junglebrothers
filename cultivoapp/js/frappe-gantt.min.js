// Versão atualizada do renderizador Gantt com cor de fundo por mês,
// texto posicionado corretamente (esquerda ou direita) e responsividade
...
window.Gantt = class {
  constructor(selector, tarefas, periodo = "Tudo") {
    ...
    let inicio, fim;
    switch (periodo) {
      case "Semana": inicio = new Date(hoje); fim = new Date(hoje); fim.setDate(fim.getDate() + 7); break;
      case "Quinzena": inicio = new Date(hoje); fim = new Date(hoje); fim.setDate(fim.getDate() + 15); break;
      case "Mes": inicio = new Date(hoje); fim = new Date(hoje); fim.setDate(fim.getDate() + 30); break;
      case "Semestre": inicio = new Date(hoje); fim = new Date(hoje); fim.setMonth(fim.getMonth() + 6); break;
      case "Ano": inicio = new Date(hoje); fim = new Date(hoje); fim.setFullYear(fim.getFullYear() + 1); break;
      default: inicio = inicioTodos; fim = fimTodos;
    }
    ...
    // fundo alternado por mês
    let mesCursor = new Date(inicio);
    mesCursor.setDate(1);
    let corMes = true;
    while (mesCursor < fim) {
      const proxMes = new Date(mesCursor.getFullYear(), mesCursor.getMonth() + 1, 1);
      const inicioX = ((mesCursor - inicio) / (fim - inicio)) * 100;
      const largura = ((proxMes - mesCursor) / (fim - inicio)) * 100;
      const bloco = document.createElement("div");
      bloco.className = corMes ? "bg-gray-100 absolute top-0 left-0 h-full" : "absolute top-0 left-0 h-full";
      bloco.style.left = `${inicioX}%`;
      bloco.style.width = `${largura}%`;
    ...
    for (let i = 0; i < semanas; i++) {
      ...
      cel.className = "text-center border py-1 bg-white";
      cel.style.width = `${larguraSemana}%`;
      cel.textContent = `S${semana}`;
    ...
    tarefas.forEach(t => {
      ...
      barra.className = "h-4 rounded";
      barra.style.background = cor;
      linha.appendChild(barra);

      const texto = document.createElement("div");
      texto.className = "absolute text-gray-800 whitespace-nowrap";
      texto.textContent = `${t.name}: ${formatarData(t.start)} → ${formatarData(t.end)}`;

      const posicaoTexto = (diasInicio / diasTotal) * 100 + (diasDuracao / diasTotal) * 100;
      if (posicaoTexto > 90) {
        texto.style.right = `${100 - posicaoTexto + 1}%`;
      } else {
        texto.style.left = `${(diasInicio / diasTotal) * 100 + (diasDuracao / diasTotal) * 100 + 0.5}%`;
      }
