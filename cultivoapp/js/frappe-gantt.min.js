...
window.Gantt = class {
  constructor(selector, tasks) {
    ...
    tasks.forEach(t => {
      ...
      if (!grupos[grupo]) {
        const cor = cores[Object.keys(grupos).length % cores.length];
        grupos[grupo] = cor;
        nomesCultivos[grupo] = t.name.split(" ")[0]; // exemplo: usar primeira palavra como nome curto
      }
    });

    // Legenda
    const legenda = document.createElement("div");
    legenda.className = "flex flex-wrap gap-4 items-center text-sm mb-4";
    Object.entries(grupos).forEach(([grupo, cor]) => {
      const item = document.createElement("div");
      item.className = "flex items-center gap-2";
      const corBox = document.createElement("div");
      corBox.style.backgroundColor = cor;
      corBox.className = "w-4 h-4 rounded";
    ...
    hojeLabel.innerHTML = `<span class='text-red-600'>📍 Hoje:</span> ${hoje}`;
    ...
    tasks.forEach(task => {
      ...
      const largura = Math.max(30, dias * 20); // largura mínima e proporção
