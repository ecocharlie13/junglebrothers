// Novo renderizador Gantt com ajuste dinâmico da posição do texto
window.Gantt = class {
  constructor(selector, tarefas, periodo = "Tudo") {
    ...
    grade.className = "overflow-hidden p-4";
    ...
    cabecalho.className = "flex text-xs font-bold text-gray-700 mb-2";
    for (let i = 0; i < semanas; i++) {
      const d = new Date(inicio);
      d.setDate(d.getDate() + i * 7);
      const semana = semanaAno(d);
      const coluna = document.createElement("div");
      coluna.className = `text-center border py-1 ${i % 2 === 0 ? 'bg-gray-100' : 'bg-white'}`;
    ...
    tarefas.forEach(t => {
      ...
      linha.className = "flex items-center text-xs mb-2 relative";
      ...
      const texto = `${t.name}: ${formatarData(t.start)} → ${formatarData(t.end)}`;
      const posicaoDireita = (diasInicio + diasDuracao) / diasTotal;

      const textoSpan = document.createElement("span");
      textoSpan.textContent = texto;
      textoSpan.className = "ml-2 text-gray-800";

      if (posicaoDireita < 0.85) {
