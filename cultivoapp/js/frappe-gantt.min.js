window.Gantt = class {
  constructor(selector, tarefas, periodo = "Tudo") {
    ...
    tarefas.forEach(t => {
      ...
      const rightBuffer = (diasInicio + diasDuracao) / diasTotal * 100;
      const buffer = (diasDuracao / diasTotal) * 100;
      if (rightBuffer + 10 > 100) {
        // Se ultrapassa o fim, joga texto à esquerda
        texto.style.right = `${100 - leftPercent + 0.5}%`;
        texto.style.left = "auto";
        texto.style.textAlign = "right";
      } else {
        // Senão, mantém à direita da barra
        texto.style.left = `${rightBuffer + 0.5}%`;
      }
