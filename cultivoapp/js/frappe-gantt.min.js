window.Gantt = class {
  constructor(selector, tarefas, periodo = "Tudo") {
    ...
    let corToggle = false;
    Object.values(mesesMap).forEach(indexes => {
      const bloco = document.createElement("div");
      bloco.className = corToggle ? "absolute top-0 left-0 bottom-0 bg-gray-100" : "absolute top-0 left-0 bottom-0 bg-white";
      bloco.style.zIndex = "0";
      bloco.style.width = `${(indexes.length / diasTotal) * 100}%`;
      bloco.style.left = `${(indexes[0] / diasTotal) * 100}%`;
    ...
    tarefas.forEach(t => {
      ...
      barra.className = "rounded h-6";
      barra.style.background = cor;
      linha.appendChild(barra);

      const texto = document.createElement("div");
      texto.className = "absolute text-gray-700 text-xs whitespace-nowrap";
      texto.textContent = `${t.name}: ${formatarData(t.start)} → ${formatarData(t.end)}`;

      const sobra = 100 - ((diasInicio + diasDuracao) / diasTotal) * 100;
      texto.style[diasInicio / diasTotal > 0.5 ? "right" : "left"] = diasInicio / diasTotal > 0.5 ? `${sobra}%` : `${(diasInicio / diasTotal) * 100 + (diasDuracao / diasTotal) * 100}%`;
      texto.style.top = "-1.25rem";
