...
inputDataInicio.addEventListener("change", () => {
  ...
  for (const bloco of blocos) {
    if (bloco.nome === "TAREFA") continue;
    const ini = new Date(inputDataInicio.value);
    ini.setDate(ini.getDate() + index * 7);
...
function atualizarColheitaEDiaAtual() {
  ...
  const ultimo = blocosValidos[blocosValidos.length - 1];
...
// 🔹 Restante igual...
// A lógica de renderizarBlocos() e os blocos interativos permanece conforme o seu último código enviado,
// com a única diferença de trocar "PROCESSAR" para "TAREFA", ajustar a lógica de datas e ocultar os controles fora do modo edição.
